...
**Shell escaping**: Values for `--where`, `--regex`, and similar options are interpreted by the shell first. Use **single-quoted** arguments when a value contains `$` (variable expansion) or `!` (history expansion in many shells): e.g. `--where 'Fees!="$0.00"'`, `--where 'Symbol!=ABC'`. Otherwise the filter may receive the wrong string and return incorrect results.
...
- **Pipelines**: Chain scripts via pipes for multi-step queries. Example — "top 5 most recently created datasets with feature X enabled":
  ...
  ```
- **Escaping in filter/where**: When `--where` (or `--regex`, etc.) values contain `$` or `!`, use single-quoted arguments (e.g. `--where 'Fees!="$0.00"'`, `--where 'Symbol!=ABC'`); otherwise the shell may alter the string and the script can return incorrect results.
