...
**Shell escaping**: Values for `--where`, `--in`, `--contains`, `--regex` are interpreted by the shell before the script sees them. Use **single-quoted** arguments when the value contains:
- **`$`** — otherwise the shell expands variables (e.g. `$0.00` can become empty).
- **`!`** — in many shells (e.g. bash) `!` inside double quotes triggers history expansion, so `--where "Symbol!=ABC"` can be mangled; use `--where 'Symbol!=ABC'` for not-equal.

Examples: `--where 'Fees!="$0.00"'`, `--where 'Symbol!=ABC'`. Alternatively escape `$` in double-quoted strings: `--where "Fees!='\$0.00'"`.
...
- **Escaping in filter/where**: When `--where` (or `--in`, `--contains`, `--regex`) values contain `$` or `!`, use single-quoted arguments (e.g. `--where 'Fees!="$0.00"'`, `--where 'Symbol!=ABC'`). The shell expands `$` and in many shells `!` triggers history expansion inside double quotes, so the script may receive a different string and return incorrect results.
