...
- **External contracts.** For provider APIs, services, SDKs, formats, licences or regulations, verify the current primary source rather than memory or secondary documentation. Record material uncertainty as a question/gate; keep implementation detail out of the PRD unless it is a product requirement.
...
- **External-contract steps**: cite the primary source and verification date; pin applicable endpoint/version, auth, transfer/pagination, schema and failure behavior. Require provider-derived evidence—a safe smoke, authorized live probe, or verified current fixture—before readiness; hand-written fakes prove only local behavior. If unavailable, mark the gap and gate rollout.
...
2. **Workspace** — if the user wasn't explicit, ask: (a) a new git worktree, (b) in place where the repo currently sits (current branch, or master/main). For (a), the orchestrator creates a **persistent** worktree on a new branch (`git worktree add <root>/<repo>-sprint-NN-slug -b sprint/NN-slug`), points the implementer at that path, and commits there — **not** the Agent tool's throwaway `isolation: worktree`, which auto-cleans and would discard the work. Resolve `<root>` first-match-wins: a path the user named → a worktree root named in project guidance (project `CLAUDE.md`, `docs/`) → the parent of an existing sprint worktree in `git worktree list` → `..`. Never place it **inside** the repo's working tree unless gitignored — the repo's own globs, linters, and test runners would sweep it up. For (b) on the default branch, branch first before any commit.
...
6. **Summary first** — before any code, print the resolved choices: target (expanded to the concrete ordered list of steps/phases in scope), workspace (the **absolute worktree path** under (a), so the user vetoes it here rather than after the fact), adversarial-pass cadence, commit cadence, implementer. This print is the final go-ahead checkpoint.
